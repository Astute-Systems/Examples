...
MODULE_DESCRIPTION("v4l2 driver module for Intersil tw686x. Version 1.1.26");
...
void tw68v_set_framerate(struct TW68_dev *dev, u32 ch, u32 n)
{
	if (n >= 0 && n < 6)
	{
		if (ch >= 0 && ch < MAX_CHANNELS)
			tw68_reg_writel(dev, DROP_FIELD_REG0 + ch, video_framerate[n][0]);
...
void BFDMA_setup(struct TW68_dev *dev, int nDMA_channel, int H, int W)
{
	...
	tw68_reg_writel(dev, (BDMA_ADDR_P_F2_0 + nDMA_channel * 8), dev->BDbuf[nDMA_channel][2].dma_addr); // P DMA page table
...
static irqreturn_t TW68_irq(int irq, void *dev_id)
{
	...
	if (!int_status && !TW68_FIFO_ERROR(fifo_status))
		return IRQ_NONE;
...
// probe function
static int TW68_probe(struct pci_dev *pci_dev,
					  const struct pci_device_id *pci_id)
{
...
#endif
	printk("812/1012[vbuf2] driver 10/31/2023 1.1.26\n");
	...
	printk(KERN_INFO "%s: subsystem: %04x:%04x, board: %s [card=%d,%s]\n",
		   ...
		   dev->board, dev->autodetected ? "autodetected" : "insmod option");
