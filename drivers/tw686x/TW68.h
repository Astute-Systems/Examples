...
#define MAX_AUDIO_CH 8
#define TW68_VERSION_CODE KERNEL_VERSION(5, 25, 0)
...
struct TW68_dev
{
	...
	struct TW68_audio_channel *audio_channels;
	/// DMA smart control
	unsigned int videoDMA_ID;
	unsigned int videoCap_ID;
	unsigned int videoRS_ID;
	unsigned int pending_dma_cmd;
	unsigned int pending_dma_en;
...
#define TW68_AUDIO_PAGE_MAX 16
#define TW68_AUDIO_PERIODS_MIN 2
#define TW68_AUDIO_PERIODS_MAX TW68_AUDIO_PAGE_MAX

struct TW68_dma_desc
{
	dma_addr_t phys;
	void *virt;
	unsigned int size;
...
struct TW68_audio_channel
{
	struct TW68_dev *dev;
	struct snd_pcm_substream *ss;
	unsigned int ch;
	struct TW68_audio_buf *curr_bufs[2];
	struct TW68_dma_desc dma_descs[2];
	dma_addr_t ptr;

	struct TW68_audio_buf buf[TW68_AUDIO_PAGE_MAX];
	struct list_head buf_list;
	spinlock_t lock;
	int current_record_volume;
};
...
#define tw_write(reg, value)                       \
	{                                              \
		writel((value), chip->bmmio + (reg));      \
		if (chip->ivytown)                         \
			udelay(10);                            \
...
static inline long tw68_reg_readl(struct TW68_dev *dev, u32 reg)
{
	return readl(dev->lmmio + (reg));
}

#define REG_TO_READBACK 0x2E
static inline void tw68_reg_writel(struct TW68_dev *dev, int reg, int val)
{
	writel((val), dev->lmmio + (reg));
	if (dev->ivytown)
		udelay(10);
	// read back to prevent caching of commands
	(void)tw68_reg_readl(dev, REG_TO_READBACK);
	if (dev->ivytown)
		udelay(25);
	return;
}
...
extern void TW68_audio_free(struct TW68_dev *dev);
...
extern int TW68_audio_init(struct TW68_dev *dev);
...
void TW68_video_irq(struct TW68_dev *dev, unsigned long requests, unsigned int pb_status,
